...
function injectStyles() {
  ...
  style.innerHTML = `
    #game-container {
      max-width: 1400px !important; margin: 0 auto !important; display: flex !important;
      flex-direction: column !important; padding: 10px !important; align-items: center !important;
...
let firewallBuffActive = false; // Prevents boss healing
...
function glitchTransition(callback) {
  ...
  function drawGlitch(intensity) {
    ...
    for (let i = 0; i < numSlices; i++) {
      ...
      ctx.fillStyle = color; ctx.globalAlpha = Math.random() * 0.6 * intensity;
...
function startRichardLoop() {
  setTimeout(() => {
    ...
    const imgs = ['assets/yourbossvar/boss-crossing.png', 'assets/yourbossvar/boss-pointing.png'];
...
═══════════════════════════════════════════════════════════════════════════ */
// Inject the HTML Overlay dynamically
...
let firewallActive = false, firewallTimer = null, virusSpawner = null, virusMover = null;
let mainframeHP = 3, firewallTimeLeft = 15;
...
function startFirewallGame() {
  ...
  document.querySelectorAll('.virus-icon').forEach(v => v.remove());

  virusSpawner = setInterval(() => {
    if (!firewallActive) return;
    const virus = document.createElement('div');
    virus.className = 'virus-icon'; virus.innerText = '👾';
    virus.style.position = 'absolute'; virus.style.fontSize = '30px'; virus.style.userSelect = 'none';
    ...
    virus.onpointerdown = (e) => { e.stopPropagation(); virus.remove(); playClickSound(); };
    arena.appendChild(virus);
  }, 600);

  virusMover = setInterval(() => {
    document.querySelectorAll('.virus-icon').forEach(v => {
      let x = parseFloat(v.style.left), y = parseFloat(v.style.top);
      let dx = 135 - x, dy = 135 - y, dist = Math.sqrt(dx*dx + dy*dy);
      
      if (dist < 15) {
        v.remove(); mainframeHP--;
        document.getElementById('firewall-hp').innerText = mainframeHP;
        ...
        v.style.left = (x + (dx / dist) * 1.5) + 'px'; v.style.top = (y + (dy / dist) * 1.5) + 'px';
      }
    });
  }, 30);

  firewallTimer = setInterval(() => {
    firewallTimeLeft--; document.getElementById('firewall-time').innerText = firewallTimeLeft;
...
function endFirewallGame(won) {
  ...
  clearInterval(firewallTimer); clearInterval(virusSpawner); clearInterval(virusMover);
  ...
  if (won) {
    ...
    popup.innerText = "FIREWALL UP! BOSS HEAL BLOCKED FOR 60s!";
    popup.style.cssText = `left:50%;top:50%;--tx:0px;--ty:-80px;--rot:0deg;color:#00ffcc;`;
    document.body.appendChild(popup); setTimeout(() => popup.remove(), 3500);
    firewallBuffActive = true;
    const bArea = document.getElementById('boss-area'); if(bArea) bArea.style.borderBottom = '5px solid #00ffcc';
...
function bindInteractions() {
  ...
  bind('btn-clock-in', 'click', () => {
    ...
    if (v) {
      myUser = v; document.getElementById('login-screen').style.display = 'none'; document.getElementById('game-container').style.display = 'flex';
  ...
  // Firewall Binding using Event Delegation
  document.addEventListener('click', (e) => {
    if (e.target.closest('#skill-firewall') || (e.target.innerText && e.target.innerText.includes('Firewall'))) startFirewallGame();
