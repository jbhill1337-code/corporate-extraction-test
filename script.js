...
const richardQuotes = [
  ...
  "MAXIMIZE YOUR OUTPUT.", "LEVERAGE THE PIPELINE.", "FAILURE IS NOT OPTIMAL.", 
  "DISRUPT THE DISRUPTION.", "SCALE IT, NOW."
];
...
/* ══ UTILS, UI & SAVE ══════════════════════════════════════════════════════ */
function createDynamicPopup(t, c, x, y) { const p = document.createElement('div'); p.className = c; p.innerText = t; p.style.left = x + 'px'; p.style.top = y + 'px'; document.body.appendChild(p); setTimeout(() => p.remove(), 3500); }
...
function endFirewallGame(won) {
  ...
  if (won) {
    createDynamicPopup("FIREWALL UP! BOSS HEAL BLOCKED FOR 60s!", 'loot-popup', window.innerWidth/2, window.innerHeight/2);
    firewallBuffActive = true; const bArea = document.getElementById('boss-area'); if(bArea) bArea.style.borderBottom = '5px solid #00ffcc';
    setTimeout(() => { firewallBuffActive = false; if(bArea) bArea.style.borderBottom = 'none'; createDynamicPopup("FIREWALL DOWN! HEALING RESTORED.", 'damage-popup', window.innerWidth/2, window.innerHeight/2); }, 60000);
  } else { createDynamicPopup("BREACH DETECTED! DEFENSE FAILED.", 'damage-popup', window.innerWidth/2, window.innerHeight/2); }
}
...
document.body.insertAdjacentHTML('beforeend', `
  <div id="encryption-minigame-overlay" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.95); z-index:200000; flex-direction:column; align-items:center; justify-content:center;">
    ...
    <div style="background:#111; border:3px solid #ff00ff; padding:20px; border-radius:10px; text-align:center; width: 400px;">
      ...
      <div style="display:flex; justify-content:center; gap:15px; margin-top:20px;">
        <button class="enc-btn" data-sym="🔺">🔺</button>
        <button class="enc-btn" data-sym="🟦">🟦</button>
        <button class="enc-btn" data-sym="🟡">🟡</button>
        <button class="enc-btn" data-sym="🟩">🟩</button>
      </div>
    ...
    <button id="encryption-close-btn" class="vapor-btn" style="margin-top:20px;">ABORT HACK</button>
  </div>
`);

let encActive = false, encTimer = null, encTimeLeft = 10;
let encTarget = [], encPlayer = [];
const encSymbols = ['🔺', '🟦', '🟡', '🟩'];
...
function startEncryptionGame() {
  ...
  document.getElementById('enc-target-seq').innerText = encTarget.join('');
  document.getElementById('enc-player-seq').innerText = '....';
  document.getElementById('enc-time').innerText = encTimeLeft;
  document.getElementById('encryption-minigame-overlay').style.display = 'flex';
...
function handleEncInput(sym) {
  if(!encActive) return;
  encPlayer.push(sym);
  document.getElementById('enc-player-seq').innerText = encPlayer.join('');
...
function endEncryptionGame(won) {
  ...
  if (won) {
    createDynamicPopup("MAINFRAME CRACKED! 3x GLOBAL DAMAGE FOR 60s!", 'loot-popup', window.innerWidth/2, window.innerHeight/2);
    encryptionMultiplier = 3.0; updateUI();
    const bArea = document.getElementById('boss-area'); if(bArea) bArea.style.filter = 'drop-shadow(0 0 20px #ff00ff)';
    setTimeout(() => { encryptionMultiplier = 1.0; updateUI(); if(bArea) bArea.style.filter = 'none'; createDynamicPopup("ENCRYPTION RESTORED. DAMAGE NORMALIZED.", 'damage-popup', window.innerWidth/2, window.innerHeight/2); }, 60000);
  } else { createDynamicPopup("TRACE COMPLETE! HACK FAILED.", 'damage-popup', window.innerWidth/2, window.innerHeight/2); }
}
