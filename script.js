...
function injectStyles() {
  ...
  style.innerHTML = `
    ...
    #boss-area { display: flex !important; align-items: flex-end !important; justify-content: center !important; gap: 40px !important; width: 100% !important; position: relative !important; overflow: visible !important; cursor: pointer !important; }
    #boss-image, #companion-image { height: 280px !important; width: auto !important; display: block !important; image-rendering: pixelated !important; }
...
/* ══ LOOT TABLE (Damage Balance) ════════════════════════════════════════════ */
// Math changed to ADDITIVE so late game doesn't break the server HP.
const lootTable = [
...
function rollLoot(x, y) {
  ...
  let pool;
  if (roll < 0.008) pool = lootTable.filter(i => i.rarity === 'legendary'); // 0.8%
  else if (roll < 0.03) pool = lootTable.filter(i => i.rarity === 'rare'); // 2.2%
  else if (roll < 0.09) pool = lootTable.filter(i => i.rarity === 'uncommon'); // 6%
  else if (roll < 0.15) pool = lootTable.filter(i => i.rarity === 'common'); // 6%
  else return;
...
function initSystem() {
  ...
  // Dynamically unlock Firewall & Encryption Skills in UI
...
/* ══ SKIP BUTTON & INTRO (Your Original Glitch Transition!) ════════════════ */
let ytPlayer = null;
...
const endIntro = () => {
  ...
  // Restored your Glitch transition effect
  glitchTransition(() => {
...
function glitchTransition(callback) {
  ...
  function animate() {
    ...
    if (progress < 0.3) {
      const intensity = progress / 0.3; glitch.style.opacity = intensity * 0.85; drawGlitch(intensity);
    } else if (progress < 0.6) {
      glitch.style.opacity = '1'; glitch.style.background = '#fff'; ctx.clearRect(0, 0, canvas.width, canvas.height); drawGlitch(1);
    } else if (progress < 0.7) {
      glitch.style.background = '#000'; glitch.style.opacity = '1'; ctx.clearRect(0, 0, canvas.width, canvas.height);
      if (frame === Math.floor(totalFrames * 0.6) + 1) callback();
    } else {
      const fadeOut = 1 - ((progress - 0.7) / 0.3); glitch.style.opacity = Math.max(0, fadeOut); drawGlitch(fadeOut);
...
// 10-Second Auto-Failsafe just in case YouTube hangs entirely
...
// Boss Heals 10k per second unless blocked by Firewall
setInterval(() => {
...
function updateUI() {
  ...
  const bc = document.getElementById('buy-click'); if (bc) bc.innerHTML = '⚔️ Upgrade Click (+2.5k) <br><span>Cost: ' + clickCost.toLocaleString() + '</span>';
  ...
  const bs = document.getElementById('buy-synergy'); if (bs) bs.innerHTML = '⚡ Synergy Boost (+10% dmg) <br><span class="cost-tag">Cost: ' + synergyCost.toLocaleString() + '</span>';
  const br = document.getElementById('buy-rage'); if (br) br.innerHTML = '🔥 Rage Fuel (faster frenzy) <br><span class="cost-tag">Cost: ' + rageCost.toLocaleString() + '</span>';
...
let phishPool = [], phishIndex = 0, phishScore = 0, phishTotal = 4;
...
function answerPhish(userSaysPhish) {
  ...
  // Clean 2.5s visual delay transition before next question
...
/* ══ FIREWALL & ENCRYPTION MINIGAMES (Injecting the HTML Safely) ═══════════ */
...
function bindInteractions() {
  // Inject the Minigame HTML blocks securely before we attach listeners
  injectMinigameHTML();

  const bind = (id, ev, fn) => { const el = document.getElementById(id); if (el) el.addEventListener(ev, fn); };

  // Skip Intro is bound safely
  ...
  // Economy fixes are applied securely here!
  ...
  // Event Delegation for the new Dynamic Minigame Buttons
